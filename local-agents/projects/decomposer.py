"""ProjectDecomposer: heuristic brief -> epics -> tasks (no LLM)."""
from dataclasses import asdict
from projects.schema import Epic, SubTask

_PATTERNS = [
    {"keywords": ["fastapi", "flask", "django"], "epic_title": "Backend Setup",
    ...
    {"keywords": ["react", "nextjs", "next.js", "vue", "angular", "svelte"], "epic_title": "Frontend Setup",
    ...
    {"keywords": ["auth", "authentication", "login", "signup", "register", "oauth", "jwt", "session"],
    ...
    {"keywords": ["database", "postgres", "postgresql", "mysql", "sqlite", "mongodb", "db", "orm", "sqlalchemy"],
    ...
    {"keywords": ["docker", "kubernetes", "k8s", "deploy", "deployment", "ci", "cd", "github actions"],
     ...
     "priority": 2, "tasks": [
         ...
         ("CI pipeline", "Configure GitHub Actions for lint, test, build.", "scaffold", 2),
    ...
    {"keywords": ["redis", "cache", "caching", "memcached"],
    ...
    {"keywords": ["celery", "worker", "queue", "task queue", "async tasks", "job", "rq"],
    ...
    {"keywords": ["cli", "command line", "command-line", "argparse", "click", "typer"],
     ...
     "priority": 1, "tasks": [
         ("CLI scaffold", "Set up CLI framework with entry point.", "scaffold", 1),
...
def _detect_type(brief_lower: str) -> str:
    if "fastapi" in brief_lower:
        return "fastapi"
    if "flask" in brief_lower:
        return "flask"
...
class ProjectDecomposer:
    """Decomposes a project brief into epics/tasks using heuristic patterns. No LLM needed."""
        ...
        for pattern in _PATTERNS:
            if any(kw in brief_lower for kw in pattern["keywords"]):
                ...
                if title in seen:
                    continue
                seen.add(title)
                tasks = [asdict(SubTask(title=t[0], description=t[1], category=t[2]))
                         for t in pattern["tasks"]]
                matched_epics.append({"title": title, "description": pattern["epic_description"],
                                      "priority": pattern["priority"], "tasks": tasks})
        if not matched_epics:
            matched_epics.append({"title": "Core Implementation", "description": brief[:300],
                                  "priority": 1,
                                  "tasks": [asdict(SubTask(title="Implement core feature",
                                                           description=f"Implement: {brief[:200]}",
                                                           category="code_gen"))]})
        matched_epics.sort(key=lambda e: e["priority"])
        return {"project_type": detected_type, "epics": matched_epics}
    ...
    def decompose_into_project(self, brief: str, name: str = "", project_type: str = "") -> dict:
        ...
        for ep in result["epics"]:
            e_dict = asdict(Epic(title=ep["title"], description=ep["description"], priority=ep["priority"]))
            e_dict["tasks"] = ep["tasks"]
            epics_out.append(e_dict)
        return {"name": name or brief[:60].strip(), "type": result["project_type"],
                "description": brief, "epics": epics_out}
