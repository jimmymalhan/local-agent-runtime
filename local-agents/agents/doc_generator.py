...
def generate_readme_section(project_path):
...
def generate_changelog_entry(task_title, files_changed):
    """Generate a changelog entry for the given task and changed files"""
...
def run(task):
    """Generate docs: mode=api|readme|changelog, path=source directory"""
    ...
    out_file = Path(path) / f"docs_{mode}_{ts}.md"
