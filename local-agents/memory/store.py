...
class MemoryStore:
    """Unified memory interface with a single shared SQLite connection."""
    ...
    def remember_task(self, task: dict, result: dict, quality: int) -> int:
        """Store completed task in episodic memory. Auto-promotes quality >= 80 to patterns."""
        ...
        if quality >= ProceduralMemory.QUALITY_THRESHOLD:
            ...
            if output:
                ...
                self.procedural.store_pattern(
                    ...
                    content=output[:2000], quality=quality,
                )
        return row_id
    ...
    def learn_pattern(self, pattern_name: str, pattern_code: str, quality: int,
                      category: str = "general", description: str = "") -> Optional[int]:
        """Store a successful pattern in procedural memory."""
        return self.procedural.store_pattern(
            name=pattern_name, category=category, description=description,
            content=pattern_code, quality=quality,
        )
