...
Table: semantic_facts(id, key, value, source, project_id, timestamp)
Table: semantic_project_maps(id, project_id, map_json, timestamp)
Table: semantic_api_specs(id, project_id, endpoint, spec_json, timestamp)
"""
import json
import sqlite3
import time
...
class SemanticMemory:
    ...
    def store_fact(self, key: str, value: str, source: str, project_id: Optional[str] = None):
        ...
        existing = conn.execute(
            "SELECT id FROM semantic_facts WHERE key = ? AND "
    ...
    def recall_facts(self, query: str, project_id: Optional[str] = None, n: int = 10) -> list:
        ...
        if not keywords:
            return []
        conditions = " OR ".join(["(LOWER(key) LIKE ? OR LOWER(value) LIKE ?)"] * len(keywords))
        params = [x for k in keywords for x in (f"%{k}%", f"%{k}%")]
        if project_id:
            params.append(project_id)
            proj_filter = " AND project_id = ?"
        else:
            proj_filter = ""
        params.append(n)
        rows = self._conn().execute(
            f"SELECT id, key, value, source, project_id, timestamp FROM semantic_facts "
            f"WHERE ({conditions}){proj_filter} ORDER BY timestamp DESC LIMIT ?",
            params,
        ).fetchall()
    ...
    def store_project_map(self, project_id: str, project_map: dict):
        ...
        conn = self._conn()
        conn.execute(
            "INSERT INTO semantic_project_maps (project_id, map_json, timestamp) VALUES (?, ?, ?) "
    ...
    def get_project_map(self, project_id: str) -> dict:
        ...
        row = self._conn().execute(
            "SELECT map_json FROM semantic_project_maps WHERE project_id = ?", (project_id,)
        ).fetchone()
        if row:
            try:
    ...
    def store_api_spec(self, project_id: str, endpoint: str, spec: dict):
        """Upsert an API endpoint spec."""
        conn = self._conn()
        conn.execute(
            "INSERT INTO semantic_api_specs (project_id, endpoint, spec_json, timestamp) VALUES (?, ?, ?, ?) "
            "ON CONFLICT(project_id, endpoint) DO UPDATE SET spec_json = excluded.spec_json, timestamp = excluded.timestamp",
