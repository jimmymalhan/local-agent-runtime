...
async def get_projects():
    """Return projects from local-agents/projects/projects.json or state."""
...
async def create_project(request: dict):
    """Create project via ProjectManager; falls back to CLI hint."""
    ...
    try:
        ...
        pm = ProjectManager()
        project = pm.create_project(name=name, **{k: v for k, v in request.items() if k != "name"})
        return {"ok": True, "project": project}
    except ImportError:
        return {"error": "ProjectManager not available", "hint": "python3 local-agents/projects/cli.py new"}
    except Exception as exc:
